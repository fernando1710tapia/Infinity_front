...
///////FTFT                cli.setCodigocomercializadora(cliente.getString("codigocomercializadora"));
    ...
    public List<Cliente> obtenerClientesPorID(String codCliente) {
        try {
            //URL url = new URL("https://www.supertech.ec:8443/infinityone1/resources/ec.com.infinity.modelo.cliente/porId?codigo="+codCliente);
            URL url = new URL(Fichero.getRUTASERVICIOSPERSISTENCIA().trim() + "ec.com.infinity.modelo.cliente/porId?codigo=" + codCliente);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setDoInput(true);
            connection.setRequestMethod("GET");
            connection.setRequestProperty("Accept", "application/json");
