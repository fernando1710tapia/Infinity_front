...
@Named
@ViewScoped
public class ActualizarGestionarPreciosBean extends ReusableBean implements Serializable {
     ...
     */
    public ActualizarGestionarPreciosBean() {
    }
    ...
    public void actualizarDatosComerProd(Comercializadoraproducto comerP) {
        try {
            ...
            if (connection.getResponseCode() == 200) {
                ...
                this.dialogo(FacesMessage.SEVERITY_ERROR, "ERROR AL ACUTALIZAR PRECIO");
    ...
    public boolean obtenerPrecioscomprobacion(Comercializadoraproducto comerP) {
        try {
            ...
            url = new URL(direcc + "codigocomercializadora=" + comerP.getComercializadoraproductoPK().getCodigocomercializadora() + "&codigoproducto=" + comerP.getProducto().getCodigo() + "&codigomedida=" + comerP.getMedida().getCodigo());
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setDoOutput(true);
                ...
                switch (listaGravamen.get(j).getGravamenPK().getCodigo()) {
                    ...
                    case "0005":
                        if (listaPrecio.get(i).getPrecio().getListaprecio().getTipo().equals("MCO")) {
                            BigDecimal mcsiva = (listaPrecio.get(i).getPrecio().getComercializadoraproducto().getMargencomercializacion().divide(new BigDecimal(1.12), 6, RoundingMode.HALF_UP)).setScale(6, RoundingMode.HALF_UP);
...
//valorIvaDividir = 1 + grabamen codigo 2 (iva).valordefecto
    ...
    public void savePrice() {
        for (int i = 0; i < listPrice.size(); i++) {
            for (int j = 0; j < listaTerminales.size(); j++) {
                addItemsPrice(i, j, listPrice, listaTerminales);
            }
        }
        reestablecer();
    }
